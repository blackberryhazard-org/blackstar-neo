...
export default {
  ...
  async run(m, { sock, group, groupMetadata, isPrefix, command, args }) {
    if (command === "sider") {
      const [option] = args;
      const now = Date.now();
      ...
      if (option === "-y") {
        ...
        return client.groupParticipantsUpdate(
          m.chat,
          inactiveMembers,
          "remove",
        );
    ...
    if (!userId) return m.reply("💭 Reply user message.");
    if (isLidUser(userId)) return m.reply("❌ Invalid user.");
    ...
    if (command === "+warn") {
      ...
      const isUserInGroup = groupMetadata.participants.some(
        (participant) =>
          participant.id === userId || participant.phoneNumber === userId,
      );
      if (!isUserInGroup) return m.reply("❌ User not found in this group.");
      const [json] = await sock.groupParticipantsUpdate(
        m.chat,
        [userId],
